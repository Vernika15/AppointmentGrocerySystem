...
import { getAvailableSlots } from "../app.logic.ts";
...
export function Form(isModal = false): HTMLFormElement {
  ...
  const formData: Partial<Appointment> = isEditing
    ? { ...state.form }
  ...
  const availableSlots =
    doctor && date
      ? getAvailableSlots(
          doctor,
          date,
          isEditing ? state.form.id ?? null : null
        )
      : [];
  ...
  const updateSlotDropdown = (newDoctor: string, newDate: string) => {
    const slots =
  ...
  if (isModal) {
    doctorSelect.addEventListener("change", (e) => {
      state.form.doctor = (e.target as HTMLSelectElement).value;
      renderApp();
    });

    dateInput.addEventListener("change", (e) => {
      state.form.date = (e.target as HTMLInputElement).value;
      renderApp();
    });
  } else {
    // Left form updates slot dropdown without re-rendering
    doctorSelect.addEventListener("change", (e) => {
      doctor = (e.target as HTMLSelectElement).value;
  ...
  form.addEventListener("submit", (e) => {
    ...
    const data = raw as unknown as Omit<Appointment, "id">;

    if (isEditing && state.form.id) {
